...
def set_hebdaily_jobs(scheduler, today=datetime.date.today(), test=False, dont_rerun=False):
    ...
    if today_folders:
        ...
        # Erev
        data = player_data(day_part='Erev', folders=today_folders)
        play_presentation(scheduler, data, test)        #Play now

        # Morning
        start_time = sunrise
        data = player_data(day_part='Morning', folders=today_folders)
        ...
        data = player_data(day_part='Afternoon', folders=today_folders)
        if scheduler: scheduler.add_job(play_presentation, 'date', run_date=start_time, args=[scheduler, data])
        logging.debug('Afternoon: %s', start_time)
    else:
        folders = ['Default',]
        if yesterday_folders:    # Yesterday (now) was a holiday, so we are motzei
            data = player_data(day_part='Motzei', folders=yesterday_folders)
        else:        # Regular day
            data = player_data(day_part='Evening', folders=folders)
        play_presentation(scheduler, data, test)        #Play now
        logging.debug('Now: %s', data.folders)

        # Morning
        start_time = sunrise
        data = player_data(day_part='Morning', folders=folders)
        if scheduler: scheduler.add_job(play_presentation, 'date', run_date=start_time, args=[scheduler, data])
        logging.debug('Morning: %s', start_time)
